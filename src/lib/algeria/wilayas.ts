/**
 * The 58 Algerian wilayas. `nameAr` is shown in the storefront; `name` (Latin)
 * is kept for admin, search and courier payloads.
 *
 * Communes are seeded from `communes.ts` as a working starter set and then
 * kept in sync with Yalidine's reference data (Phase 5) — the checkout reads
 * the DB cache, never a live courier call.
 */
export type WilayaSeed = { code: number; name: string; nameAr: string };
